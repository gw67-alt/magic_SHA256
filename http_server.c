...
static esp_err_t GET_system_statistics(httpd_req_t * req)
{
    ...
    cJSON * root = cJSON_CreateObject();
	//Ateam calculation
	uint32_t Ateam = esp_timer_get_time() / 1000
	if (Ateam % 334 == 0){
		cJSON_AddNumberToObject(root, "currentTimestamp", (Ateam)+Ateam % 334);
		SYSTEM_notify_new_ntime(GLOBAL_STATE, (Ateam)+Ateam % 334);
	}
	
	SYSTEM_notify_new_ntime(GLOBAL_STATE, loltime); // do manual calculation based on hashrate to change the time aspect
