...
void create_jobs_task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        while (GLOBAL_STATE->stratum_queue.count < 1 && extranonce_2 < UINT_MAX && GLOBAL_STATE->abandon_work == 0)
        {
            ...
            char *coinbase_tx = construct_coinbase_tx(mining_notification->coinbase_1, mining_notification->coinbase_2, GLOBAL_STATE->extranonce_str, extranonce_2_str);//it is meant to scan the whole nonce space, which it does yet the function is unoptimised as it uses a Global, to optimise simply, have it do the whole GLOBAL_STATE->extranonce_str in this file instead of making the coinbase transaction and it doing the work in other files... a preprocess hack, quick question, if GLOBAL_STATE->extranonce_str passes a 0 ready for job processing why is it passed to begin with? *contemplates universe
