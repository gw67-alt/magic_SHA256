...
def calculate_sha256_with_library(data):
    """Calculates SHA-256 on CPU (for comparison and small tasks)."""
...
GUESS_TRIGGER_COUNT = 3 # Number of samples before attempting a guess
        ...
        if self.app_state.current_state == AppState.STATE_TRACKING:
            ...
            if game_state["credits"] > 0:
                ...
                if is_below:
                    ...
                    sha = calculate_sha256_with_library("GeorgeW" + str(self.guess_trigger_sample_counter))
                    if sha.startswith(prefix): 
                        self.m+=1
                        print("Ready state:", self.n,"Success:", self.m, "Nonce:", self.guess_trigger_sample_counter, sha)
            ...
            if self.guess_trigger_sample_counter >= 10000000000:
                self.guess_trigger_sample_counter = 0
